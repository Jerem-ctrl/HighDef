...
def plotFreqNorm(signalEntree, Fe_Hz, c, titre):
    """trace le spectre d'amplitude NORMALISE du signal signalEntree, qui est échantillonné à Fe_Hz Hz. c indique la couleur souhaité (ex : 'b' pour bleu). titre indique le titre."""
    longueur=len(signalEntree)
    specOrig=abs((1/longueur)*fftshift(fft(signalEntree)))
    freq = arange(-1/2,1/2,1/longueur) 
    plot(freq, specOrig, c)
    grid()
    title(titre, fontsize=16)
    xlabel('Frequence (Hertz)', fontsize=16)
